...
def append_id(filename):
    ...
    return "".join(parts[:-1])+ '_encryped' + '.' + parts[-1]
...
def encrypt_pdf(input_file: str, password: str):
    ...
    pdf_reader = PdfReader(open(input_file, 'rb'), strict=False)
    ...
    pdf_writer.encrypt(user_password="", owner_pwd=password, use_128bit=True,permissions_flag=0b0100)
...
if __name__ == '__main__':
    ...
    encrypt_decrypt_file(
        input_file=filename, password="dsflksdjf#", 
        action=args['action'], level=args['level'], output_file=append_id(filename)
    )
